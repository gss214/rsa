...
class MillerRabin:
    ...
    def single_test(self, number: int, witness: int) -> bool:
        ...
        :return: bool, True if number is probably prime, False otherwise
        """
        exp, rem = number - 1, 0
    ...
    def is_prime(self, number: int, k=40) -> bool:
        """
        Test a number for primality using the Miller-Rabin primality test
        :param number: int, the number to be tested for primality
        :param k: int, the number of iterations of the single_test function to perform
        :return: bool, True if number is probably prime, False otherwise
        """
        if number <= 1:
            return False
